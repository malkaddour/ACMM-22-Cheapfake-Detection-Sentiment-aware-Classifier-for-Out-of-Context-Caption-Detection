...
class MaskRCNNExtractor(nn.Module):
    ...
    def forward(self, imgs, bboxes, bbox_classes):
        ...
        #    [self.maskrcnn_predictor(img.cpu(), target) for img, target in zip(imgs, targets)])
        #print("images is a tensor: ", torch.is_tensor(imgs))
        #print("targets is a tensor: ", torch.is_tensor(targets))
        #for img, target in zip(imgs, targets):
        #    print("IMG is a tensor: ", torch.is_tensor(img))
        #    print("TARGET is a tensor: ", torch.is_tensor(target))
        #    print(target)
        #    print("transformed TARGET is a tensor: ", torch.is_tensor(torch.as_tensor(target)))
        #bbox_feats = torch.cat(
        #    [self.maskrcnn_predictor(img.cpu(), target) for img, target in zip(imgs, targets)]
        #)
        '''for img, target in zip(imgs, targets):
            curpred = self.maskrcnn_predictor(img.cpu().numpy(), target)
            #print(curpred)
            print("self shit is a tensor: ", torch.is_tensor(curpred))
            maskcnn = curpred[0]
            maskcnn = maskcnn['instances']
            #print(maskcnn)
            #print(img.shape)
            print(maskcnn.pred_boxes)
            print("are boxes tensors? ", torch.is_tensor(maskcnn.pred_boxes))'''
        bbox_feats = torch.stack(
        ...
        #    try:
        #        bbox_feats = torch.stack(
        #            [self.maskrcnn_predictor(torch.as_tensor(img.cpu()), target) for img, target in zip(imgs, targets)])
        #    except:
        #        bbox_feats = torch.stack(
        #            [self.maskrcnn_predictor(img.cpu(), target) for img, target in zip(imgs, targets)])
        #bbox_feats = torch.stack(
